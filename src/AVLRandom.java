/* 
 * Writes random numbers into files and into array. Inserts into AVL tree.
    ...
    for(int i = 1; i <= 5; i++) {	
    	...
    	try (PrintWriter file = new PrintWriter(
    		...
    		for (int j =0, k = 1; k <= input; k++) {
    			int num =  new Random().nextInt(input);
    			arr[j] = num;
    			while(arr[j] != num)
    				file.println(arr[j]);
    			j++;
    	...
    	//prints when the current file has completed inserting the random numbers
    	System.out.println();
